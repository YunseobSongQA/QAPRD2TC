...
기획서를 붙여넣으면 표준 QA 양식의 테스트 케이스를 자동 생성해 주는 정적 웹 도구입니다.
서버나 DB 없이 브라우저에서 바로 동작하며, 입력 데이터는 외부로 전송되지 않습니다.

## 주요 기능

- 파일 드래그앤드롭: PPT(.pptx), 워드(.docx), 엑셀(.xlsx/.xls), CSV, 텍스트 파일을 끌어다 놓으면 내용을 추출해 변환
- 기획서(PRD) 텍스트 직접 입력 후 테스트 케이스(TC) 자동 변환
- 주요 기능 자동 인식: 로그인, 회원가입, 검색, 결제, 장바구니, 업로드, 권한 등
- 정상 / 예외 / 경계 케이스 자동 구성
- 표준 QA 양식 컬럼: `TC ID · 대분류 · 중분류 · 시나리오 · 사전조건 · 테스트 단계 · 기대결과 · 우선순위 · 유형`
- 엑셀(.xlsx), CSV, Markdown 내보내기 및 클립보드 복사
- AI 모드: 본인 Anthropic API 키 입력 시 브라우저에서 Claude(`claude-opus-4-8`)를 직접 호출해 더 정교한 TC 생성 (키는 서버에 저장되지 않음)
- 라이트/다크 모드 전환(설정 기억), 모바일·PC 반응형
...
> 자동 생성 결과는 초안입니다. 실제 적용 전 QA 담당자의 검토를 권장합니다.
...
- [x] Phase 2 — AI 모드(본인 Anthropic API 키로 Claude `claude-opus-4-8` 직접 호출)
- [ ] TC 항목 직접 편집/추가 기능
